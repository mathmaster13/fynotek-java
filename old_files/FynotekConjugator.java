// First iteration of the Fynotek conjugator and FynotekWord class.
import java.util.HashMap;
import java.util.Scanner; // Only necessary for user input.
public class FynotekConjugator { // All this class does is deliver the prompts to the user. All of the actual processing is done by the FynotekWord class.
  static Scanner input = new Scanner(System.in);
  public static void main(String[] args) {
    FynotekWord word = new FynotekWord(prompt("Enter a fynotek word:  "));
    if (getPartOfSpeech() == 'n') {
      word.setProper(getNounType());
      System.out.println(word.nounCase(getCase()));
    } else {
      System.out.println(word.verbTense(getTense(), getHypothetical()));
    }
  }
...
// That's all the user input code. The rest of these are functions to make said user input code easier to read.
  ...
  public static char getPartOfSpeech() {
    char partOfSpeech;
    do {
      System.out.print("What part of speech is this word? (N = noun, V = verb):  ");
      partOfSpeech = Character.toLowerCase(input.next().charAt(0));
    } while (!(partOfSpeech == 'n' || partOfSpeech == 'v' ));
    return partOfSpeech;
  }
  ...
  public static boolean getNounType() {
    char proper;
    do {
      System.out.print("Is this noun proper or common? (P = proper, C = common):  ");
      proper = Character.toLowerCase(input.next().charAt(0));
    } while (!(proper == 'p' || proper == 'c' ));
    return (proper == 'p');
  }
  public static char getTense() {
    char verbTense;
    do {
      System.out.print("What tense is this word? (P = present, A = past, F = future, G = gnomic):  ");
      verbTense = Character.toLowerCase(input.next().charAt(0));
    } while (!(verbTense == 'p' || verbTense == 'a' || verbTense == 'f' || verbTense == 'g'));
    return verbTense;
  }
  public static boolean getHypothetical() {
    char hypothetical;
    do {
      System.out.print("Is this a hypothetical tense? (Y = yes, N = no):  ");
      hypothetical = Character.toLowerCase(input.next().charAt(0));
    } while (!(hypothetical == 'y' || hypothetical == 'n' ));
    return (hypothetical == 'y');
  }
}
